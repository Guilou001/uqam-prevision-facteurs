...
Comment lire cette figure : chaque point est la variation du taux de chômage d'un mois à l'autre, en points de pourcentage. La bande colorée est la fenêtre de février 2020 à juin 2021 que le travail met de côté dans sa seconde variante. Le pic d'avril 2020, 10,4 points, vaut à lui seul 11,6 fois le plus fort mouvement mensuel observé hors de cette fenêtre, 0,9 point en janvier 1975. C'est ce qui explique pourquoi la question de le retirer se pose.
...
+ *Prévoir de façon directe.* Pour l'horizon h, les deux autorégressifs ne gardent que les retards h et suivants, puis prédisent le mois qui suit la fin de leur fenêtre. Celui d'horizon 4 ne voit donc jamais les trois mois qui précèdent immédiatement ce qu'il prédit, et un test le vérifie en recomposant la prévision à la main depuis les quatre valeurs retardées. Les quatre autres modèles décalent moins. L'ARMA ne décale que son retard autorégressif, et garde les résidus des deux mois précédents. L'ARDL et le modèle à facteurs décalent de même leur retard du chômage, mais gardent leurs séries explicatives sans décalage. Le VAR n'est pas direct du tout : il est prolongé de h mois à partir des dernières observations.
...
Comment lire ce tableau, en quatre constats. D'abord, cinq des six modèles battent le repère en moyenne, et le sixième, le modèle à facteurs, fait pire : résumer 119 séries en deux composantes n'aide pas à prévoir cette série-là. Ensuite, l'ARDL est le plus bas de façon constante : il tient le meilleur score du tableau à neuf des douze horizons, et 0,864 en moyenne. Le VAR, lui, tient le meilleur score du tableau à l'horizon 1 et passe au-dessus du repère aux horizons 2 et 4. Sa colonne ne se compare pas d'un horizon à l'autre, pour la raison donnée dans les limites. Puis, la moyenne historique tient la ligne la plus plate des sept, autour de 0,931 : ne rien modéliser du tout coûte 7 % de moins que le repère. Enfin, l'écart entre le meilleur et le pire modèle vaut 0,143 en moyenne, ce qui est petit au regard de 61 mois de test. Le travail de 2021 le disait déjà, en proposant des tests statistiques comme extension.

#figure(image("../results/figures/erreurs_par_horizon.png", width: 100%), caption: [Les erreurs par horizon])

Comment lire cette figure : six courbes, une par modèle, le repère n'ayant pas la sienne. L'horizon est en abscisse, l'erreur rapportée au repère en ordonnée. La ligne pointillée à 1 est le repère lui-même, l'autorégressif d'ordre 1 à l'horizon 1. La ligne bleue presque horizontale est la moyenne historique, qui ne dépend pas de l'horizon. Les courbes qui zigzaguent sont celles des modèles estimés, et leur zigzag mesure surtout le bruit de l'exercice, pas une propriété des horizons. La courbe du VAR fait exception : son décalage d'horizon, décrit dans les limites, y contribue aussi.
...
#table(
  ...
  inset: 5pt,
    [*Modèle*],
    [*Erreur quadratique moyenne, avec la Covid*],
    [*Sans la Covid*],
...
#table(
  ...
  inset: 5pt,
    ...
    [Le VAR n'est pas direct : à l'horizon H il prévoit le mois t + H - 1, et son erreur est mesurée contre le mois t],
    [mesuré ; le carnet de 2021 apparie de la même façon, donc la réplication est fidèle, et c'est la comparaison d'un horizon à l'autre qui perd son sens pour cette colonne],
